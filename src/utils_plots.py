...
def plot_f1(f1s, graph_path, title):
    ...
    # key_order = ['train-F1', 'valid-F1', 'test-F1']
    # key_mapping = {key: i for i, key in enumerate(key_order)}
    # ordered_f1s = collections.OrderedDict(sorted(f1s.items()))
    ordered_f1s = sorted(f1s.items(), key=operator.itemgetter(0))
    # ordered_f1s = sorted(f1s.items(), key=lambda x: key_mapping[x)
    # for i in range(len(ordered_f1s)):
    #     label, f1_list = ordered_f1s[i]
    ...
    # train_max = max(f1s['train-F1'])
    # valid_max = max(f1s['valid-F1'])
    # test_max = max(f1s['test-F1'])
    # plt.text( f1s['train-F1'].index(train_max), train_max + 5 ,'train: {:.2f}'.format(train_max) )
    # plt.text( f1s['valid-F1'].index(valid_max), valid_max + 5 ,'valid: {:.2f}'.format(valid_max) )
    # plt.text( f1s['test-F1'].index(test_max), test_max + 5 ,'train: {:.2f}'.format(test_max) )
